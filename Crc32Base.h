...
#ifndef CRC_WPMC_CRC32BASE_H
#define CRC_WPMC_CRC32BASE_H

#include "CrcBase.h"

class Crc32Base : public CrcBase<uint32_t> {
public :
    Crc32Base(uint32_t poly) : CrcBase(poly) {}

    uint32_t crc_s1(uint8_t *data, uint64_t length) {
    ...
    uint32_t crc_s2(uint8_t *data, uint64_t length) {
    ...
    uint32_t crc_s4(uint8_t *data, uint64_t length) {
    ...
    uint32_t crc_s8(uint8_t *data, uint64_t length) {
