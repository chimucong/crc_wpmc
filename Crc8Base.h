...
#ifndef CRC_WPMC_CRC8BASE_H
#define CRC_WPMC_CRC8BASE_H

#include "CrcBase.h"

class Crc8Base : public CrcBase<uint8_t> {
public:
    Crc8Base(uint8_t poly) : CrcBase(poly) {}

    uint8_t crc_s1(uint8_t *data, uint64_t length) {
        uint8_t *p = (uint8_t *) data;
        uint8_t *end = (uint8_t *) (data + length);
        uint8_t crc = 0;
        while (p < end) {
            crc ^= *p;
            crc = table_8[crc & 0xff];
            p++;
        }
        return crc;
    }

    uint8_t crc_s2(uint8_t *data, uint64_t length) {
    ...
    uint8_t crc_s4(uint8_t *data, uint64_t length) {
    ...
    uint8_t crc_s8(uint8_t *data, uint64_t length) {
